...
class IQArray(object):
    ...
    def export_to_wav(self, filename, num_channels, sample_rate, long_record):
        from urh.dev import config
        if long_record:
            f = wave.open(pickle.load(open(QDir.currentPath()+config.MY_PATH+"/path_IQRecord.pickle", "rb"))+ "/"+ filename, "w")
        else:
            f = wave.open(QDir.currentPath()+"/"+filename, "w")
