...
class DirectoryTreeView(QTreeView):
    ...
    def create_directory(self):
        ...
        dir_name, ok = QInputDialog.getText(self, self.tr("Створити директорію"), self.tr("Назва директорії"))

        if ok and len(dir_name) > 0:
            if not model.mkdir(index, dir_name).isValid():
                QMessageBox.information(self, self.tr("Створити директорію"), self.tr("Неможливо створити директорію"))
    ...
    def remove(self):
        ...
        if not ok:
            QMessageBox.information(self, self.tr("Видалити"),
                                    self.tr("Неможливо видалити {0}".format(model.fileName(index))))
