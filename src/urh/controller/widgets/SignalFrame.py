...
from PyQt5.QtCore import pyqtSignal, QPoint, Qt, QMimeData, pyqtSlot, QTimer, QDir
...
class SignalFrame(QFrame):
    ...
    #-----------------------
    my_data = pyqtSignal(str)
    my_pulse_data = pyqtSignal(str)
    my_period_data = pyqtSignal(str)
    my_next_table = pyqtSignal()
    my_name_data = pyqtSignal(str)
    my_table = pyqtSignal(str)
    ...
    def __init__(self, proto_analyzer: ProtocolAnalyzer, undo_stack: QUndoStack, project_manager, parent=None):
        ...
        # self.choise_dir = QDir.currentPath()
        # self.r_dat = ''
        self.ui.labelModulation.hide()
        
        # self.ui.cbModulationType.hide()
        self.ui.btnSetNoise.setEnabled(False)
        #-------------------------------------------
        self.ui.btnSetNoise.setToolTip(self.tr("Встановлення рівня шуму"))
        self.ui.btnScreen.setToolTip(self.tr("Знімок сигналу"))
        self.ui.btnCloseSignal.setToolTip(self.tr("Закрити записаний сигнал"))
        #-------------------------------------------
        ...
        if self.signal is not None:
            ...
            if self.signal.wav_mode:
                ...
                self.ui.lSignalTyp.setText("Сигнал")
        ...
        #----------------------------------
        # self.on_btn_autodetect_clicked()
    ...
    def create_connects(self):
        ...
        self.ui.btnScreen.clicked.connect(self.my_screen)
        self.ui.gvSignal.selection_width_changed.connect(self.my_set_btn_vsbl)
        ...
        if self.signal is not None:
            ...
            #--------------
            self.signal.message_indices.connect(self.middle_time)
        ...
        # self.ui.pushButton16.setVisible(self.ui.gvSignal.something_is_selected)

        self.ui.btnSetNoise.clicked.connect(self.on_set_noise_in_graphic_view_clicked)
...
#------------
    ...
    def my_screen(self):
        ...
        screenshot = screen.grabWindow(self.ui.splitter.winId())
        screenshot.save(pickle.load(open(QDir.currentPath()+config.MY_PATH+"/path_bd.pickle", "rb"))[:-22]+'{0}.jpg'.format(self.signal.name),'jpg')
        QMessageBox.information(self, "Знімок", "Знімок успішно створено")
# ------------
    ...
    def __set_duration(self):  # On Signal Sample Rate changed
        ...
        if self.signal:
            # t = round(num_samples / self.signal.sample_rate * 1e6)
            t = num_samples / self.signal.sample_rate
            # self.ui.lDuration.setText(str(t) + " мкс")
            self.ui.lDuration.setText(Formatter.science_time(t))
            #-----------------------------
            pulse = Formatter.science_time(t)
            self.my_data.emit(pulse)
            self.hand_pulse = pulse
#---------------------------------------------------------------
            mcs = 'мкс'
            ms = 'мс'
            ns = 'нс'
            ...
            if str(self.hand_pulse).replace(',', '.')[-2:] == ns:
                # print("НаноСекунда")
                b = float(str(self.hand_pulse).replace(',', '.')[:-3])
                bb = b / 1000
                a = bb
                # print('Наносекунда в Микросекундах', '{:.7f}'.format(a))
            elif str(self.hand_pulse).replace(',', '.')[-2:] == ms:
                # print("Милисекунда")
                b = float(str(self.hand_pulse).replace(',', '.')[:-2])
                bb = b * 1000
                a = bb
                # print('Милисекунда в Микросекундах', '{:.7f}'.format(a))
            else:
                # print("Микросекунда")
                c = float(str(self.hand_pulse).replace(',', '.')[:-3])
                a = c
                # print("Микросекунда в микросекундах",a)

            procpd = (float(a) * self.ui.spin_proc.value()) / 100
            self.pulse1 = float(a) - procpd  # pulse duration
            self.pulse2 = float(a) + procpd  # pulse duration

            procfolperiod = (float(a) * self.ui.spin_proc.value()) / 100
            self.period1 = float(a) - procfolperiod  # period duration
            self.period2 = float(a) + procfolperiod  # period duration

            self.conn = sqlite3.connect(QDir.currentPath()+config.MY_PATH+"/database.db")

            sql = "SELECT * FROM el_intel where Pulse_duration_mcs >='{}' and Pulse_duration_mcs <='{}' or Follow_period >= '{}' and Follow_period <= '{}' order by Pulse_duration_mcs and Follow_period desc ".format(
                self.pulse1, self.pulse2, self.period1, self.period2)
            ...
            for i in range(self.ui.tableWidget.rowCount()):
                print(i, self.ui.tableWidget.item(i, 5).text(), "----", self.ui.tableWidget.item(i, 6).text())
                pul = float(self.ui.tableWidget.item(i, 5).text()) if self.ui.tableWidget.item(i, 5).text() != '' else 0
                per = float(self.ui.tableWidget.item(i, 6).text()) if self.ui.tableWidget.item(i, 6).text() != '' else 0

                if a == pul:
                    self.ui.tableWidget.item(i, 1).setBackground(QtGui.QColor(0, 200, 0, 100))
                    self.ui.tableWidget.item(i, 5).setBackground(QtGui.QColor(0, 200, 0, 100))

                if a == per:
                    self.ui.tableWidget.item(i, 1).setBackground(QtGui.QColor(0, 200, 0, 100))
                    self.ui.tableWidget.item(i, 6).setBackground(QtGui.QColor(0, 200, 0, 100))

# ---------------------------------------------------------------


            # print("Time pulse duration (328 str) SignalFrame.py-> {}".format(pulse))
    ...
    @pyqtSlot(list)
    def middle_time(self,message_indices_data):
        mid_pulse =[]
        for i in message_indices_data:
            mid_pulse.append(i[1]-i[0])

        mid_pulse.sort()
        mid_pulse = mid_pulse[::-1][:-5]
        #-----
        try:
            mid_pulse = sum(mid_pulse) / len(mid_pulse)
        except ZeroDivisionError:
            mid_pulse = 0
        #-----

        a = round(mid_pulse/self.signal.sample_rate * 1e6)

        self.ui.label_2.setText(str(a) + ' мкс')
        # ----------------------
        # self.my_pulse_data.emit(a)
        # ----------------------
        mid_period = []
        for i in range(message_indices_data.__len__() -1 ):
            mid_period.append(message_indices_data[i+1][0] - message_indices_data[i][0])

        mid_period.sort()
        mid_period = mid_period[::-1][:-2]
        

        try:
            mid_period = sum(mid_period) / len(mid_period)
        except ZeroDivisionError:
            mid_period = 0

        b = round(mid_period/self.signal.sample_rate * 1e6)

        # print("------- SEVERAL----------")
        # print(b, ' period ')
        # print(a, ' pulse ')

        self.ui.label_4.setText(str(b) + ' мкс')

        # self.my_period_data.emit(b)
        self.x = a
        self.y = b
        ...
        self.pulse1 = float(a) - procpd  # pulse duration
        self.pulse2 = float(a) + procpd  # pulse duration
        print(self.pulse1, self.pulse2)

        procfolperiod = (float(b) * self.ui.spin_proc.value()) / 100
        self.period1 = float(b) - procfolperiod  # period duration
        self.period2 = float(b) + procfolperiod  # period duration
        print(self.period1, self.period2)
        ...
        for row_number, row_data in enumerate(data):
            ...
            for column_number, data in enumerate(row_data):
                self.ui.tableWidget.setItem(row_number, column_number, QTableWidgetItem(str(data)))
            # print(self.ui.tableWidget.model().data(self.ui.tableWidget.currentIndex()))
        self.conn.close()
    ...
    def clickedEl_intel(self, r, ):
        self.name = self.ui.tableWidget.model().index(r, 1).data()
        print("Name -> {} SIGNAL FRAME".format(self.ui.tableWidget.model().index(r, 1).data()))
        self.my_name_data.emit(self.name)

    def r(self, data):
        self.r_dat = data

    def path_bd(self, data):
        self.path_database_session = data
        print(self.path_database_session)

    def record(self):

        try:
            shutil.copy(QDir.currentPath() + "/" + self.signal.name + ".wav", self.r_dat + "/" + self.signal.name + ".wav")
        except Exception as E:
        ...
        try:
            freq = self.signal.name.split('-')[4].replace("_", "")[:-3]
        except Exception as E:
            freq = 0
        try:
            band = self.signal.name.split('-')[6].replace("_", ".")[:-3]
        except Exception as E:
            band = 0

        if self.signal.name == None:
            try:
                cur.execute(
                    '''INSERT INTO table2 (Date, Freq, Spectrum_width, Type_modulation, Pulse_duration, Follow, Comment) values (?,?,?,?,?,?,?)''',
                    (timestamp, freq, band, mod, self.x, self.y, self.name))
            except Exception as E:
                QMessageBox.information(self, "База данних", "         Відсутня база данних\nНеобхідно створити нову сессію")
        else:
            try:
                cur.execute(
                        '''INSERT INTO table2 (Date, Freq, Spectrum_width, Type_modulation, Pulse_duration, Follow, Comment, path) values (?,?,?,?,?,?,?,?)''',
                        (timestamp, freq, band, mod, self.x, self.y, self.name, self.r_dat + "/" + self.signal.name + ".wav"))
            except Exception as E:
                QMessageBox.information(self, "База данних", "         Відсутня база данних\nНеобхідно створити нову сессію")
        self.conn.commit()
        cur.close()
        self.conn.close()
        # ---------
        self.my_next_table.emit()
        a = '1'
        self.my_table.emit(a)
    ...
    def on_btn_close_signal_clicked(self):
        try:
            os.remove(QDir.currentPath()+"/"+self.signal.name + ".wav")
        except Exception as E:
    ...
    def on_set_noise_in_graphic_view_clicked(self):
        if self.ui.gvSignal.something_is_selected:
            self.setCursor(Qt.WaitCursor)
    ...
    def on_btn_autodetect_clicked(self):
        ...
        self.unsetCursor()
        if not success:
            Errors.generic_error(self.tr("Автоматизований аналіз не вдався"),
                                 self.tr("Помилка автоматизованого зчитування параметрів сигналу."))
            self.ui.label_4.setText(str(0) + ' мкс')
            self.ui.label_2.setText(str(0) + ' мкс')
    ...
    @pyqtSlot()
    def my_set_btn_vsbl(self):
        if abs(self.ui.gvSignal.selection_area.width) > 0:
            self.ui.btnSetNoise.setEnabled(True)
        else:
            self.ui.btnSetNoise.setEnabled(False)
    ...
    def contextMenuEvent(self, event: QContextMenuEvent):
        ...
        menu = QMenu()
        apply_to_all_action = menu.addAction(self.tr("Детальніше про станцію")) #Apply values (BitLen, 0/1-Threshold, Tolerance) to all signals
        menu.addSeparator()
    ...
    @pyqtSlot(str)
    def clicketable(self, r):
        n = (self.ui.tableWidget.model().index(r, 1).data())
        print(n)
